      ...
      >
        ...
        >
          ...
          <template v-if="zoneEditMode && activeMachineTab === 'idu'">
            ...
            <button
              type="button"
              class="create-zone-btn"
              @click="createTopLevelZone"
            >
          ...
          <template v-else>
            <div class="zone-panel">
              ...
              <div class="zone-tree">
                <el-tree
                  :data="currentZoneTreeData"
                  :props="zoneTreeProps"
                  node-key="id"
                  :highlight-current="true"
                  :expand-on-click-node="false"
                  :default-expanded-keys="defaultExpandedZoneKeys"
                  @node-click="handleCurrentZoneNodeClick"
                  @node-expand="handleCurrentZoneNodeExpand"
                ...
                >
                  ...
                  >
                    <span class="zone-tree-content">
                      <input
                        v-if="showBatchControl"
                        type="checkbox"
                        :checked="isCurrentZoneNodeSelected(data)"
                        @click.stop="toggleCurrentZoneNodeSelect(data)"
                      />
                      <input
                        v-if="isEditingOutdoorZoneNode(data)"
                        ref="outdoorZoneEditInput"
                        class="zone-tree-edit-input"
                        type="text"
                        v-model="outdoorZoneEditValue"
                        @click.stop
                    ...
                    <button
                      ...
                      "
                      type="button"
                      class="zone-tree-edit-btn"
                      title="编辑"
                      @click.stop="editOutdoorZoneNode(data)"
                      @mousedown.prevent
                    >
        ...
        <main class="main-panel">
          <div class="main-content">
            ...
            <div class="filter-panel">
              <!-- 第一行筛选条件 -->
              <el-row :gutter="20" class="filter-row-wrapper">
                <el-col :span="8">
                ...
                <el-col :span="!expandedFilters ? 7 : 8">
                  <div class="filter-box">
                    <span class="filter-label">{{
                      activeMachineTab === "idu" ? "所属系统" : "所属系统"
                    }}</span>
                ...
                <el-col :span="!expandedFilters ? 7 : 8">
                  <div class="filter-box">
                    <span class="filter-label">{{
                      activeMachineTab === "idu" ? "实时状态" : "运行状态"
                    }}</span>
                    <el-cascader
                      v-if="activeMachineTab === 'idu'"
                ...
                <el-col :span="2" class="expand-col">
                  <button
              ...
              <el-row
                :gutter="20"
                v-if="expandedFilters && activeMachineTab === 'idu'"
              ...
              >
                ...
                <el-col :span="6">
                  <div class="filter-box">
                    ...
                    <el-select
                      v-model="filters.indoorModel"
                      placeholder="请选择"
                      size="small"
                      class="expanded-select indoor-model-select"
                    >
                      <el-option
                        v-for="item in indoorModelOptions"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                      ></el-option>
                ...
                <el-col v-if="expandedFilters" :span="2" class="expand-col">
                  <button
                    type="button"
                    class="expand-link"
                    @click="expandedFilters = !expandedFilters"
              ...
              <el-row
                :gutter="20"
                v-if="expandedFilters && activeMachineTab === 'odu'"
                class="filter-row-wrapper"
              >
                <el-col :span="8">
                  <div class="filter-box">
                    ...
                    <el-select
                      v-model="filters.outdoorMuteMode"
                      placeholder="请选择"
                      size="small"
                ...
                <el-col :span="!expandedFilters ? 7 : 8">
                  <div class="filter-box">
                    <span class="filter-label">合约状态</span>
                    <el-select
                      v-model="filters.outdoorContractStatus"
                      placeholder="请选择"
                      size="small"
                      class="expanded-select contract-status-select"
                    >
                ...
                <el-col :span="!expandedFilters ? 7 : 8">
                  <div class="filter-box">
                    <span class="filter-label">设备机型</span>
                    <el-select
                      v-model="filters.outdoorDeviceModel"
                      placeholder="请选择"
                      size="small"
            ...
            <div
              v-if="zoneEditMode || showBatchControl"
              class="edit-top-actions"
            >
            ...
            <div v-else style="padding: 20px 0">
              <div class="action-row" v-if="!showBatchControl">
                <div class="action-spacer"></div>
                <div class="actions">
                  <button
                    v-if="activeMachineTab === 'idu' && viewMode === 'card'"
                    type="button"
                    class="action-btn secondary"
                  ...
                  <el-button
                    :disabled="selectedDeviceFlag"
                    type="primary"
                    :class="
                      selectedDeviceFlag ? 'disabled_btn' : 'action-btn_ssss'
                    "
                    @click="openBatchControl"
                  >
                    批量控制
                  </el-button>
                </div>
            ...
            <div v-if="zoneEditMode" class="edit-layout">
              <div class="edit-transfer">
                <button type="button" class="transfer-btn disabled">
                  移出
                </button>
              ...
              <div class="edit-card-grid">
                <div
                  v-for="item in indoorEditCards"
                  :key="item.id"
                  class="device-card edit-card"
                >
                  <div class="card-title-row with-check">
                    <div class="card-title-wrap">
                      <img
                        src="https://static-btri.midea.com/mfs/2676/1723456813979.png"
                        alt="indoor-icon"
                      />
                    </div>
            ...
            <div v-else-if="viewMode === 'card'" class="card-grid">
              ...
              >
                <div class="card-title-row">
                  ...
                  <div v-if="showBatchControl" class="card-check">
                    <input
                      type="checkbox"
                  ...
                  <!-- -->
                  <div
                    v-if="selectedDevice && selectedDevice.id === item.id"
                    class="icon-check"
                  >
                    <i class="el-icon-check icon-check-icon" />
                  </div>
                </div>
              ...
              <table class="device-table">
                <thead>
                  <tr>
                    <th>
                      {{
                        activeMachineTab === "idu" ? "室内机编号" : "室外机编号"
                      }}
                    </th>
                    <th>设备名称</th>
                    <th>运行状态</th>
                    <th>
                      {{ activeMachineTab === "idu" ? "设备地址" : "所属系统" }}
                    </th>
                    <th>
                      {{ activeMachineTab === "idu" ? "系统" : "机型" }}
                    </th>
                    <th>
                      {{ activeMachineTab === "idu" ? "附属舰" : "静音模式" }}
                    </th>
                    <th>
                      {{
                        activeMachineTab === "idu" ? "内机型号" : "模式优先设置"
                      }}
                    </th>
          ...
          <div class="batch-control-container" v-if="showBatchControl">
              ...
              <template v-if="activeBatchTab === 'normal'">
                ...
                <div class="control-section">
                  ...
                  <div class="temp-control">
                    ...
                    <el-slider
                      v-model="batchControlData.temperature"
                      :min="16"
                      :max="30"
                      :step="0.5"
                      class="temp-slider"
                    ...
                    <span class="temp-value"
                      >{{ batchControlData.temperature }}°C</span
                    >
                  </div>
                </div>
                ...
                <div class="control-section">
                  ...
                  <div class="wind-control">
                    ...
                    <el-slider
                      v-model="batchControlData.windLevel"
                      :min="1"
                      :max="7"
                      :step="1"
                      class="wind-slider"
                    ...
                    <span class="wind-value"
                      >{{ batchControlData.windLevel }}档</span
                    >
                  </div>
                </div>
              </template>
              ...
              <template v-if="activeBatchTab === 'lock'">
                ...
                <div class="control-section">
                  <div class="section-title">
                    模式锁定
                    <span class="tip"
                      >(部分内机不支持锁定送风功能，不会影响送风模式锁定控制)</span
                    >
                  </div>
                  <div class="control-buttons">
                    <button
                  ...
                  <div class="control-buttons">
                    ...
                    <button
                      :class="[
                        ...
                        {
                          active:
                            batchControlData.disableController === 'unlocked',
                        },
                      ]"
                    ...
                    <button
                      :class="[
                        ...
                        {
                          active:
                            batchControlData.disableController === 'locked',
                        },
                      ]"
          ...
          <div class="batch-control-container" v-if="selectedDeviceFlag">
            <div class="drawer-header">
              <div class="header-left">
                <img
                  ...
                  "
                  :alt="
                    activeMachineTab === 'idu' ? 'indoor-icon' : 'outdoor-icon'
                  "
                />
              ...
              >
                <div class="related-header">
                  <span class="related-title"
                    >室外机名称: {{ selectedDevice.name }}</span
                  >
                </div>
            ...
            <div v-else class="edit-name-content">
              ...
              <div class="smart-naming">
                ...
                <div
                  v-for="(group, key) in smartNamingOptions"
                  :key="key"
                  class="smart-group"
                >
          ...
          <div class="modal-content guide-content">
            ...
            <div class="guide-section">
              ...
              <div class="guide-action-row">
                ...
                <span
                  >选中一个分区后，点击此按钮可新建选中对象的子集分区。</span
                >
              </div>
            </div>
        ...
        >
          <div class="modal-header">
            ...
            <button
              type="button"
              class="close-btn"
              @click="showSmartNamingDialog = false"
            >
              ×
            </button>
          </div>
        ...
        <div v-if="showSortDialog" class="modal-card sort-modal" @click.stop>
          ...
          <div class="modal-content">
            <div class="sort-condition-list">
              <div
                v-for="(item, index) in sortConditions"
                :key="index"
              ...
              >
                <el-select
                  ...
                  @change="onSortFieldChange(index)"
                  placeholder=""
                >
                  <el-option
                ...
                <i
                  class="el-icon-delete sort-delete"
        ...
        <div class="batch-content">
          <div class="control-section">
            <div class="section-title">
              <input
                type="checkbox"
                :checked="selectedDevices.length === currentCards.length"
          ...
          <div class="delete-device-list">
            <div
              v-for="item in currentCards"
              :key="item.id"
              class="delete-device-item"
            >
              <input
                type="checkbox"
                :checked="isDeviceSelected(item.id)"
                @click="toggleDeviceSelect(item.id)"
              />
              <div class="delete-device-info">
                <span class="delete-device-name">{{ item.name }}</span>
                <span class="delete-device-detail"
                  >交流电表：{{ item.acPower || "-" }}</span
                >
                <span class="delete-device-detail"
                  >直流电表：{{ item.dcPower || "-" }}</span
                >
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
      ...
      this.$http
        ...
        .then((res) => {
          ...
          if (deviceList && Array.isArray(deviceList)) {
            if (this.activeMachineTab === "idu") {
              this.indoorZones = this.transformDeviceToDeviceZones(
                deviceList,
                "IDU"
              );
            } else {
              this.outdoorZones = this.transformDeviceToDeviceZones(
                deviceList,
                "ODU"
              );
            }
    ...
    openDeviceDetail(device) {
      this.selectedDeviceFlag = true;
      this.iconBtnFlag = this.showSortDialog = false;
    ...
    confirmBatchControl() {
      const obj = {
        ...
        device_name: "1:COM9-2-10-4-S,29:COM9-2-11-1-E",
      };
      this.$http
        .post("/api/device/control-idu", this.batchControlData)
        .then((res) => {
...
.expand-toggle-row .expand-link,
.collapse-toggle-row .expand-link {
  display: flex;
  align-items: center;
  gap: 6px;
  padding: 6px 16px;
...
.expand-toggle-row .expand-link:hover,
.collapse-toggle-row .expand-link:hover {
  background: #e8f3ff;
}
...
.device-card {
  ...
  .icon-check {
    ...
    bottom: -1px;
    right: -1px;
    border-color: transparent #536282 #536282 transparent;
    border-style: solid;
    border-width: 10px;
...
.filter-row {
  grid-template-columns:
    minmax(420px, 1.55fr) minmax(300px, 1fr) minmax(300px, 1fr)
    auto;
  gap: 22px 20px;
  align-items: center;
}
...
@media (max-width: 1600px) {
  ...
  .outdoor-filter-rows .filter-row {
    grid-template-columns:
      minmax(360px, 1.2fr) minmax(260px, 1fr) minmax(260px, 1fr)
      auto;
  }
