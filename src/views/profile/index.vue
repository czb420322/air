        ...
        <!-- <iconify-icon
          ...
          :icon="
            expandedMenu === item.id
              ? '@mbt20240226:ibuilding:chevron-down'
              : '@mbt20240226:ibuilding:chevron-right'
          "
          class="expand-icon"
        ></iconify-icon> -->
      </div>
    ...
    <div class="right-content">
        ...
        <div class="form-container">
          <el-form :model="securityForm" label-width="120px">
            <el-form-item label="修改密码" label-position="left">
              <el-button type="text" @click="openPasswordModal"
                >点击修改密码</el-button
              >
            </el-form-item>
      ...
      <div v-if="activeMenu === 'myCompany'" class="content-section">
        ...
        <div class="company-header">
          <el-button type="primary" @click="showJoinModal = true"
            >加入新公司</el-button
          >
        </div>
          ...
          <el-table :data="companyList" border>
            ...
            <el-table-column align="center" label="操作">
              <template slot-scope="scope">
                <el-button
                  type="text"
                  @click="showExitConfirmModal(scope.row.id)"
                  >退出公司</el-button
                >
              </template>
            </el-table-column>
    ...
    return {
      ...
      menus: [
        { id: "basic", name: "基本设置", icon: "@mbt20240226:ibuilding:user" },
        {
          id: "security",
          name: "安全设置",
          icon: "@mbt20240226:ibuilding:lock",
        },
        {
          id: "company",
          name: "公司管理",
          icon: "@mbt20240226:ibuilding:building",
          children: true,
        },
      ],
      companySubMenus: [{ id: "myCompany", name: "我的公司" }],
      companyList: [],
      form: {
        name: this.$store.state.user.name,
        bindAccount: "",
        registerTime: this.$store.state.user.createTime,
      },
      securityForm: {
        phone: "138****8888",
      },
  ...
  methods: {
    getCompanyList() {
      const obj = {
        ...
        size: this.pageSize,
        company_name: "",
        main_admin_id: this.$store.state.user.id,
      };
      this.$http
        .get("/api/company/list", obj)
        .then((res) => {
          const { code, data } = res;
          console.log("🚀 ~ data:", data.list);
          if (code !== 200) {
            this.$message.error("获取公司列表失败，请稍后重试");
            return;
          }
    ...
    updateInfo() {
      this.$http
        .post("/api/admin/username/update", {
          new_username: this.form.name,
    ...
    submitJoin() {
      ...
      const obj = {
        company_name: this.joinForm.company,
        main_admin_name: this.$store.state.user.name,
        status: 1,//status: 状态：1-正常, 0-禁用/退出
      };

      this.$http
        .post("/api/company/join", obj)
        .then(() => {
    ...
    showExitConfirmModal(companyId) {
      ...
      this.$http.post("/api/company/delete", obj).then((res) => {
        const { code } = res;
        if (code !== 200) {
          this.$message.error("退出公司失败，请稍后重试");
          return;
        }
        this.$message.success("退出公司成功");
      });
    },
    confirmExit() {
      this.companyList = this.companyList.filter(
        (item) => item.id !== this.exitCompanyId
      );
